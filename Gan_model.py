...
class GAN(object):
    def __init__(self,sess,input_height = 256,input_width = 256,crop = True,batch_size = 8,sample_num = 64,
        output_height = 256,output_width = 256,y_dim = None,z_dim = 100,gf_dim = 64,df_dim = 64,gfc_dim = 1024,
        ...
        if not self.y_dim:
            self.g_bn3 = batch_norm(name = 'd_bn3')
    ...
    def build_model(self):
        ...
        self.g_loss = tf.reduce_mean(sigmoid_cross_entropy_with_logits(self.D_logits_,tf.ones_like(self.D)))
    ...
    def train(self,config):
        ...
        for epoch in xrange(config.epoch):
            ...
            for idx in xrange(0,batch_idx):
                ...
                _,summary_str = self.sess.run([d_optim,self.d_sum],feed_dict = {})##??有问题

                self.writer.add_summary(summary_str,counter)

                _,summary_str = self.sess.run([g_optim,self.g_sum],feed_dict = {self.z:batch_z,self.y:batch_images})
