...
class GAN(object):
	...
	def __init__(self,sess,input_height = 256,input_width = 256,crop = True,batch_size = 8,sample_num = 4,
		...
		self.z_dim = z_dim # the dimention of uniform random sampler
	...
	def discriminator(self,images,projection,reuse = False):
		...
		with tf.variable_scope("descriminator") as scope:
			...
			h1 = lrelu(tf.nn.lrn(conv2d(h0,128,name = 'd_h1_conv')))
			h2 = lrelu(tf.nn.lrn(conv2d(h1,256,k_h = 3,k_w = 3,name = 'd_h2_conv')))
			h3 = lrelu(tf.nn.lrn(conv2d(h2,512,k_h = 3,k_w = 3,name = 'd_h3_conv')))
			h4 = lrelu(tf.nn.lrn(conv2d(h3,128,k_h = 3,k_w = 3,d_h = 1,d_w = 1,name = 'd_h4_conv')))

			h5 = tf.reshape(h4,[self.batch_size,-1])
			h6 = lrelu(tf.nn.lrn(linear(h5,32,'d_h6_lin')))
			h7 = linear(h6,1,'d_h_lin')
	...
	def generator(self,z,y):
		...
		with tf.variable_scope('generator') as scope:
			...
			self.z_con_y_5 = deconv2d(z_con_y_4,[self.batch_size,512,512,3],5,5,name ='g_con_h5')
			#z_con_y_5 = tf.nn.relu(self.g_bn5(self.z_con_y_5))
			z_con_y_5 = tf.nn.tanh(self.g_bn5(self.z_con_y_5))
			return z_con_y_5
