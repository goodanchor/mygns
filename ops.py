...
class batch_norm(object):
    ...
    def __call__(self,x,train = True):
        return tf.contrib.layers.batch_norm(x,decay = self.momentum,updates_collections = None,epsilon = self.epsilon,scale = True,
                is_training = train,scope = self.name)

def conv_cond_concat(x,y):
    x_shapes = x.get_shape()
    y_shapes = y.get_shape()
    return concat([x,y*tf.ones([x_shape[0],x_shapes[1],x_shapes[2],y_shapes[3]])],3)
...
def deconv2d(input_,output_shape,k_h = 5,k_w = 5,d_h = 2,d_w = 2,stddev = 0.02,name = 'deconv2d',with_w = False):
    with tf.variable_scope(name) as scope:
        ...
        try:
            ...
            deconv = tf.nn.deconv2d(input_,w,output_shape = output_shape,strides = [1,d_h,d_2,1])
        
        biases = tf.get_variable('biases',shape = [output_shape[-1]],initializer=tf.constant_initializer(0.0))
